...
$gestion="SELECT artistas_urbanos.id_artistas,artistas_urbanos.aka,usuarios.numero_unico_registro, usuarios.id_usuario, usuarios.licencia_contrato ,parroquias.parroquia FROM artistas_urbanos
INNER JOIN usuarios ON artistas_urbanos.usuarios_id_usuario=usuarios.id_usuario
INNER JOIN parroquias ON usuarios.parroquias_id_parroquia=parroquias.id_parroquia WHERE usuarios.licencia_contrato=1";
            ...
            if(mysqli_num_rows($run_gestion) > 0)
            {
                while($row = mysqli_fetch_assoc($run_gestion))
                {
                    echo '<tr>
                            ...
                            <td>'.$row['parroquia'].'</td>
                            <td>
                            <a href="ver_consulta_artista.php?id_artista='.$row['id_artistas'].'" >Ver Artista</a>
                          </td>
                        </tr>';
                }
            }
